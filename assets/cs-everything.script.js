        ...
        start: function(){
          $( ".home-slideshow .slides li" ).each(function() {
            if($(this).hasClass("flex-active-slide")){
        	  $(this).find( "div.caption-group" ).addClass("fadeIn animated").removeClass("not-animated");
              $(this).find( "div.flex-action" ).addClass("fadeIn animated").removeClass("not-animated");
            }	
            else{
              $(this).find( "div.caption-group" ).removeClass("fadeIn animated").addClass("not-animated");  
...
function HandleCarousel(){
  ...
  if($("#footer_partner-content").length){
...
function handleScrollTop() {
  ...
  $("#scroll-to-top, .backtotop").click(function (e) {
    e.preventDefault();
  ...
  if(jQuery().gMap){
    if($('#contact_map').length){
      $('#contact_map').gMap({
        ...
        markers:[
          {
            address: '186 Royal College St London NW1 9NN',
            html: '_address'
          }
        ]
      });
...
function toggleTagsFilter(){ 
  if($( '#showTagsFilter' )){
  $( '#showTagsFilter' ).unbind().click(function(e){
        if($(this).hasClass('closed')){
          $(this).removeClass('closed');
          $('#tags-filter-content').removeClass('tags-closed');
          $(this).html("Filter <i class='fa fa-angle-down'></i>");
        }else{
          $(this).addClass('closed');
      	  $('#tags-filter-content').addClass('tags-closed');
          $(this).html("Filter <i class='fa fa-angle-up'></i>");
        }
      });
    }  
}
...
function togglePriceFilter(){ 
  if($( '#showPriceFilter' )){ 
      $( '#showPriceFilter' ).unbind().click(function(e){
        e.preventDefault();
        if($(this).hasClass('closed')){
          $(this).removeClass('closed');
          $('#price-filter-content').removeClass('tags-closed');
          $(this).html("Filter by Price <i class='fa fa-angle-down'></i>");
        }else{
          $(this).addClass('closed');
      	  $('#price-filter-content').addClass('tags-closed');
          $(this).html("Filter by Price <i class='fa fa-angle-up'></i>");
        }
      });
    } 
}
function toggleSizeFilter(){ 
  if($( '#showSizeFilter' )){ 
      $( '#showSizeFilter' ).unbind().click(function(e){
        e.preventDefault();
        if($(this).hasClass('closed')){
          $(this).removeClass('closed');
          $('#size-filter-content').removeClass('tags-closed');
          $(this).html("Filter by Size <i class='fa fa-angle-down'></i>");
        }else{
          $(this).addClass('closed');
      	  $('#size-filter-content').addClass('tags-closed');
          $(this).html("Filter by Size <i class='fa fa-angle-up'></i>");
        }
      });
    } 
}
...
$(window).ready(function($) {
  ...
  $("a.fancybox").fb({
    ...
    'titleShow': false,
    overlayColor: '#000000',
    overlayOpacity: 0.2,width:800, height:500,autoSize:true,autoResize:true
  });
